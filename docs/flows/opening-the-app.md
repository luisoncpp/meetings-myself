...
`reconnect()` reruns steps 3–6 after folder selection, the health banner **Try again** button, or sync recovery (plan 0008). `storeHealth()` only returns the last assessment; it does not reopen the engine.
